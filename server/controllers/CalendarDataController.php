...
Tools::init();
define('_TABLE_','calendar_data');
class CalendarDataController {
    public function findAll(){
        $collection = array();
		$sql = Tools::getDB()->query('SELECT id, title, start, end, start_time, end_time, url, auth FROM '._TABLE_.' ORDER BY start DESC');
    ...
    public function findByUser(User $user){
        $collection = array();
		$sql = Tools::getDB()->query('SELECT id, title, start, end, start_time, end_time, url, auth FROM '._TABLE_.' WHERE auth = '.$user->GetId());
		if ($res = $sql->fetch_array()) {
			do {
				$tmp = new CalendarData();
				$tmp->SetId($res['id']);
	...
	public function addCalendarEvent(CalendarData $calendarData){
		...
		$sql = 'INSERT INTO '._TABLE_.'(title, start, auth, end, start_time, end_time, url) VALUES('.Tools::cleanToDB($calendarData->GetTitle(), true).', '.Tools::cleanToDB($calendarData->GetStartDate(), true).', '.$calendarData->GetAuth().','.$optionalParams.')';
	...
	public function removeCalendarEvent(CalendarData $calendarData){
		$retorno = false;
		$sql = 'DELETE FROM '._TABLE_.' WHERE id = '.$calendarData->GetId().';';
		if (Tools::getDB()->query($sql)){
			$retorno = true;
		}
		return $retorno;
